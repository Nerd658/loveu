...
## 2024-07-26 - Managing Disabled States on Delayed Transitions
**Learning:** When interactive elements trigger delayed visual transitions or animations, users may click multiple times before the transition finishes, queuing duplicate timeouts or execution paths.
**Action:** Immediately set `disabled = true` and `aria-disabled="true"` upon the first click (and explicitly check for these states to return early) to prevent rapid double-clicks.
