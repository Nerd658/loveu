...
## 2023-10-27 - Accessible Typewriter Animations
**Learning:** Using `aria-live="polite"` on a container during a character-by-character typewriter animation causes excessive verbosity for screen readers, announcing each character individually or fragmenting the text.
**Action:** Remove `aria-live` from the animating container. Instead, set the full text as the `aria-label` on the focusable parent container when it is opened/focused, and apply `aria-hidden="true"` to the animating child nodes so the screen reader announces the content smoothly and completely once.
