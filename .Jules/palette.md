...
## 2024-07-06 - Custom Cursor Feedback on Disabled Elements
**Learning:** When using custom cursors mapped to interactive element states (`:has(:hover)`), elements relying on `aria-disabled="true"` (instead of native `:disabled`) can trigger false interactive feedback if not explicitly excluded.
**Action:** Always include `:not([aria-disabled="true"]):not(:disabled)` in CSS selectors that apply interactive visual cues like custom cursor scaling, ensuring screen reader and visual states align perfectly.
