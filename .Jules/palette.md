...
## 2024-08-15 - Preventing Duplicate Executions on Delayed Transitions
**Learning:** When interactive elements trigger delayed visual transitions or animations, rapid double-clicks can queue duplicate timeouts or execution paths, leading to erratic behavior like double-firing typewriter animations or focus stealing.
**Action:** Immediately set `disabled = true` and `aria-disabled="true"` upon the first click (and check for these states to return early) to prevent rapid double-clicks from queuing duplicate timeouts or execution paths.
