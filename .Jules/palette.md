...
## 2024-06-25 - Custom Cursor Interactive States vs aria-disabled
**Learning:** When interactive elements rely on `aria-disabled="true"` instead of native `:disabled` attributes, custom cursors triggering via `:has(:hover)` will incorrectly show interactive feedback (like scaling up) even when the element is conceptually disabled.
**Action:** Ensure CSS-driven visual feedback (like custom cursors triggering via `:has(:hover)`) explicitly checks for `:not([aria-disabled="true"])` to prevent incorrect interactive cues.
