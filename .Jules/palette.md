...
## 2024-06-05 - Interactive Cues on ARIA Disabled Elements
**Learning:** Using `aria-disabled="true"` preserves focusability for screen readers, but CSS interactive cues (like custom `:has(:hover)` cursors) can inadvertently trigger unless explicitly scoped to exclude disabled states.
**Action:** Always append `:not([aria-disabled="true"])` and `:not([disabled])` to interactive CSS selectors (especially global ones like custom cursors) to ensure visual feedback accurately reflects the element's operable state.
