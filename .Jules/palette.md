...
## 2024-05-24 - Prevent Rapid Double-Click Queued Timeouts
**Learning:** When interactive elements trigger delayed visual transitions or animations, rapid double-clicks can queue duplicate timeouts or execution paths (e.g., triggering a typing animation multiple times concurrently), leading to severe visual bugs.
**Action:** Immediately set `disabled = true` (and `aria-disabled="true"`) upon the first click, and add a check for these states at the top of the event listener to return early and prevent redundant executions.
