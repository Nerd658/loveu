...
## 2024-06-13 - Custom Cursor Feedback on ARIA Disabled Elements
**Learning:** In applications using custom DOM-based cursors mapped via `:has(:hover)` to interactive roles, elements disabled via `aria-disabled="true"` still trigger the interactive cursor state, providing false affordances to users.
**Action:** Ensure that global custom cursor hover rules explicitly exclude disabled states (e.g., `:not([aria-disabled="true"])` and `:not(:disabled)`) to maintain accurate visual feedback for custom UI elements.
