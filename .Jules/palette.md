...
## 2024-06-10 - Cursor Feedback on Disabled ARIA Elements
**Learning:** When interactive elements rely on `aria-disabled="true"` instead of native `:disabled` attributes, CSS-driven visual feedback (like custom cursors triggering via `:has(:hover)`) will incorrectly show interactive cues if the disabled state isn't explicitly excluded.
**Action:** Ensure custom cursors explicitly check for `:not([aria-disabled="true"])` on all target selectors to prevent misleading interactive cues on functionally disabled elements.
