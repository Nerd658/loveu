...
## 2024-06-02 - Disabled State Feedback on Custom Cursors
**Learning:** When using custom cursors mapped to interactive hover states via `:has(:hover)`, the cursor can incorrectly show interactive feedback (like scaling up) on disabled elements if they rely on `aria-disabled="true"` rather than native `:disabled` attributes.
**Action:** Always include `:not([aria-disabled="true"])` and `:not(:disabled)` in CSS `:has()` pseudo-class selectors that control interactive cursor feedback to ensure it matches the actual element state.
