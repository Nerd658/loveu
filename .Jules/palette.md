...
## 2024-06-23 - Managing Custom Cursors with ARIA Disabled States
**Learning:** When interactive elements rely on `aria-disabled="true"` instead of native `:disabled` attributes, CSS-driven visual feedback (like custom cursors triggering via `:has(:hover)`) will incorrectly show interactive cues on disabled elements if they don't explicitly check for the ARIA state.
**Action:** Ensure CSS rules for custom cursors explicitly check for non-disabled states using `:not([aria-disabled="true"])` and `:not(:disabled)` to prevent misleading interactive cues.
