...
## 2025-01-24 - Explicit ARIA-disabled Custom Cursor Feedback
**Learning:** Custom CSS cursor scaling via `:has(:hover)` does not automatically respect `aria-disabled="true"`, causing screen-reader-disabled elements to still trigger visual interactive feedback on hover.
**Action:** Always append `:not([aria-disabled="true"])` and `:not(:disabled)` inside `:has()` pseudo-class selectors when targeting interactive elements for visual feedback overrides.
