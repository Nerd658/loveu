...
## 2024-05-18 - Making Canvas Buttons Accessible
**Learning:** When using `<canvas>` elements as interactive buttons in this codebase, screen readers and keyboard users lack native communication of interactive states and state changes.
**Action:** Explicitly provide visual hover/focus states via CSS and manually update `aria-disabled` and `aria-label` attributes to ensure interactive states and state changes are communicated to screen readers.
