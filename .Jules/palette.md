...
## 2024-06-06 - Custom Cursor Disabled States
**Learning:** When custom interactive cursors rely on the CSS `:has(:hover)` pseudo-class targeting interactive elements, they can mistakenly trigger on elements that are logically disabled via `aria-disabled="true"`.
**Action:** Always append `:not(:disabled):not([aria-disabled="true"])` to interactive element selectors inside `:has()` for custom cursor hover states to prevent false affordances.
