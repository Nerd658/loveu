...
## 2024-07-25 - Managing State on Delayed Transitions
**Learning:** When interactive elements trigger delayed visual transitions or animations, rapid double-clicks can queue duplicate timeouts or execution paths.
**Action:** Immediately set `disabled = true` and `aria-disabled="true"` upon the first click, check for these states to return early, and ensure CSS hover effects explicitly check for `:not([aria-disabled="true"])` or `:not(:disabled)` to give visual feedback that the element is inactive.
