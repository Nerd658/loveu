...
## 2024-06-04 - Prevent False Affordances on Custom Cursors
**Learning:** Custom global cursors driven by CSS `:has(:hover)` must explicitly exclude elements with `aria-disabled="true"`. Otherwise, they create false active state affordances that confuse users into thinking disabled elements are still interactive.
**Action:** Always append `:not([aria-disabled="true"])` to interactive element selectors (like button, a, [role="button"]) when defining global hover effects for custom UI elements.
