...
## 2024-05-18 - Double-Click Prevention on Async UI
**Learning:** Rapid double-clicks on interactive elements that trigger delayed visual transitions can queue duplicate timeouts, leading to erratic UI state.
**Action:** Immediately apply `disabled = true` and `aria-disabled="true"` upon the first click (and check for these states to return early) on buttons triggering async animations or transitions.
