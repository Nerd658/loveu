...
## 2024-07-20 - Preventing Duplicate Interactions on Delayed Transitions
**Learning:** When interactive elements trigger delayed visual transitions or animations (e.g., using `setTimeout`), rapid double-clicks can queue duplicate execution paths, causing race conditions and visual bugs like overlapping typewriter characters or repeated animations.
**Action:** Immediately set `disabled = true` and `aria-disabled="true"` upon the first interaction, and add a check for these states to return early at the top of the event handler.
