...
## 2024-06-19 - Interactive Cursors and Disabled States
**Learning:** When interactive elements rely on `aria-disabled="true"` instead of native `:disabled` attributes, CSS-driven visual feedback (like custom cursors triggering via `:has(:hover)`) will incorrectly show interactive cues on disabled elements.
**Action:** Ensure CSS-driven visual feedback explicitly checks for `:not([aria-disabled="true"])` to prevent incorrect interactive cues.
