...
## 2024-05-24 - Custom Cursor Hover States
**Learning:** Custom cursors relying on CSS `:has(:hover)` to trigger visual feedback (like scaling) incorrectly apply to elements that are semantically disabled via `aria-disabled="true"` if the pseudo-class does not explicitly filter them out.
**Action:** Always include `:not(:disabled):not([aria-disabled="true"])` within the `:has()` selector for custom cursor hover states to prevent false interactive cues on disabled elements.
