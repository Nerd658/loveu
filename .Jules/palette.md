...
## 2024-07-25 - ARIA-Disabled State Parity for Custom Cursors
**Learning:** CSS-driven custom cursors using `:has(:hover)` can falsely signal interactivity on elements that are functionally disabled but lack native `:disabled` attributes.
**Action:** Always append `:not([aria-disabled="true"])` to custom interactive selectors in CSS to ensure visual cues match the element's accessibility state.

## 2024-07-25 - Decorative Character Accessibility
**Learning:** Decorative characters (like '✦') within text can create confusing readout for screen readers if not explicitly hidden.
**Action:** Wrap decorative characters within text elements in `<span aria-hidden="true">` to prevent redundant or confusing audio readout.
