...
## 2024-05-18 - Typewriter Accessibility
**Learning:** When implementing typewriter or character-by-character animations, using `aria-live` on the text container creates excessive screen reader verbosity as it announces every character sequentially.
**Action:** Do not use `aria-live`. Instead, set the full text as the `aria-label` on the focused parent container and use `aria-hidden="true"` on the animating child nodes so the content is announced smoothly once.
