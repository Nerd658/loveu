...
## 2024-05-24 - Prevent duplicate triggers on delayed animations
**Learning:** Rapid double-clicks on interactive elements that trigger delayed visual transitions or animations (like `#envelope` opening) can queue duplicate timeouts, causing race conditions in typing animations or CSS class toggles.
**Action:** Immediately set `disabled = true` and/or `aria-disabled="true"` upon the first click and check for these states to return early, preventing unintended rapid execution paths.
